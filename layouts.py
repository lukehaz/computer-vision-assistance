...
def first_page_layout():
    layout = [
        [sg.Image(filename='logo.png', size=(900, 200))],  
...
def detection_page_layout():
    layout = [
        [sg.Radio('Object Detection', "RADIO2", default=True, key='object_detection', tooltip='Perform object detection'), sg.Radio('Segmentation', "RADIO2", key='segmentation', tooltip='Perform image segmentation')],
        [sg.Checkbox('Enable Speed Detection', key='enable_speed_detection', default=False)],
        [sg.InputText(key='video', readonly=True, do_not_clear=True, enable_events=True, size=(40, 1)), sg.FileBrowse('Browse Video', file_types=(('Video Files', '*.mp4'), ('All Files', '*.*')), tooltip='Choose a video file')],
